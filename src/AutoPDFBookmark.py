...
re_font_size = r'([0-9.]+)pt'
re_head_title = r'h(\d+)'
re_chapter_code = r'(\d+\.\d+\.\d+)|(\d+\.\d+)|(\d+\.)'

pdf_input_name = 'src/System Card Kevin-Zach.pdf'
css_input_name = 'src/markdownhere.css'
...
def usage():
    ...
    print ('Usage:')
    print ('-f, --pdf []: specify the input .pdf file name, Kevin-Zach-output as default')
...
if __name__ == '__main__':
                ...
                for key,values in  css_list.items():
                    ...
                    if match_obj: #get selector named hxxx
                        ...
                        if 'font-size' in values and 'font-family' in values: #only match font-size and font-family
                            ...
                            # print(values)
                            if match_obj and abs(float(match_obj.group(1)) - float(context['size'])) <= 0.02:
                                print(bmk_level, context["text"])
    ...
    doc.set_toc(toc)
    pdf_input_name_new = pdf_input_name.split("/", 1)[1]#.rsplit(".", 1)[0] + '_new' + '.pdf'
