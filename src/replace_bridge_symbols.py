...
def generate_anchor_tags(input):
    ...
    while re.search(pattern, input):
        ...
        input = re.sub(
            pattern, f"{heading_level} {anchor_tag} {heading_text}\n", input, count=1
        )  # replace heading with anchor tag
    return input


filename = "Kevin-Zach"
# Open the file in read and write mode
with open(f"src/{filename}.md", "r+", encoding="UTF-8") as file:
